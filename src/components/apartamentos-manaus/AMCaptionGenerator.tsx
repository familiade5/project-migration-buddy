...
function buildCaption(data: AMPropertyData): string {
  ...
  if (data.floor) titleParts.push(`${data.floor}° Andar`);
  ...
  if (data.isRental) {
    ...
    if (data.salePrice > 0) {
      const financingText = data.acceptsFinancing
        ...
        : 'À vista';
      lines.push(`💰 Valor de venda ${formatCurrency(data.salePrice)} ${financingText}`);
    }
    if (data.subsidy > 0) {
  ...
  lines.push(`🖥 www.facebook.com/ApartamentosManaus 🖥`);
