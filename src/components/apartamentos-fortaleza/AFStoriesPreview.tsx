/**
 * AFStoriesPreview — Seletor de 4 temas (3 com 3 stories, 1 com 5 slides independentes)
 * Designs idênticos ao AM com logo AF.
 */
...
export function AFStoriesPreview({ data, photos }: AFStoriesPreviewProps) {
  ...
  const slidesByTheme: Record<ThemeId, React.ReactElement[]> = {
    1: [
      <AFStory1_T1_Curiosity data={data} photo={p0} />,
      <AFStory1_T1_Reveal data={data} photos={photos} />,
      <AFStory1_T1_CTA data={data} photo={pLast} />,
    ],
    2: [
      <AFStory2_T2_Curiosity data={data} photo={p0} />,
      <AFStory2_T2_Reveal data={data} photos={photos} />,
      <AFStory2_T2_CTA data={data} photo={pLast} />,
    ],
    3: [
      <AFStory3_T3_Curiosity data={data} photo={p0} />,
      <AFStory3_T3_Reveal data={data} photos={photos} />,
      <AFStory3_T3_CTA data={data} photo={pLast} />,
    ],
    4: [
  ...
  const handleExportTheme = async () => {
    ...
    try {
      ...
      a.download = `af-stories-${theme.name}-${data.title || 'imovel'}.zip`;
      a.click();
  ...
  const handleExportAllThemes = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `af-todos-stories-${data.title || 'imovel'}.zip`;
      a.click();
      URL.revokeObjectURL(a.href);
    ...
    <div className="space-y-4">
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              const a = document.createElement('a');
              a.href = url;
              a.download = `af-story-${String(safeStory + 1).padStart(2, '0')}-${label.toLowerCase().replace(' ', '-')}.png`;
              a.click();
      ...
      <div className="fixed -left-[9999px] top-0 pointer-events-none" aria-hidden="true">
        ...
        <div ref={t1r0} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory1_T1_Curiosity data={data} photo={photos[0]} />
        </div>
        ...
        <div ref={t1r2} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory1_T1_CTA data={data} photo={photos[photos.length - 1] ?? photos[0]} />
        </div>
        {/* Theme 2 */}
        <div ref={t2r0} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory2_T2_Curiosity data={data} photo={photos[0]} />
        </div>
        ...
        <div ref={t2r2} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory2_T2_CTA data={data} photo={photos[photos.length - 1] ?? photos[0]} />
        </div>
        {/* Theme 3 */}
        <div ref={t3r0} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory3_T3_Curiosity data={data} photo={photos[0]} />
        </div>
        ...
        <div ref={t3r2} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory3_T3_CTA data={data} photo={photos[photos.length - 1] ?? photos[0]} />
        </div>
        ...
        <div ref={t4r1} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory4_T4_Slide2 data={data} photo={photos[0]} photos={photos} />
        </div>
        <div ref={t4r2} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory4_T4_Slide3 data={data} photo={photos[0]} photos={photos} />
        </div>
        ...
        <div ref={t4r4} style={{ width: STORY_W, height: STORY_H, overflow: 'hidden' }}>
          <AFStory4_T4_Slide5 data={data} photo={photos[photos.length - 1] ?? photos[0]} photos={photos} />
        </div>
      </div>
    </div>
  );
