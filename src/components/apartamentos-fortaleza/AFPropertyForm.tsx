...
interface AFPropertyFormProps {
  data: AFPropertyData;
  onChange: (data: AFPropertyData) => void;
}

const propertyTypes = ['Apartamento', 'Casa', 'Casa em Condomínio', 'Cobertura', 'Terreno', 'Sala Comercial'];
const inputClass = 'h-10 border-gray-300 focus:border-[#0C7B8E] bg-white text-gray-900 placeholder:text-gray-400 text-sm';
const textareaClass = 'border-gray-300 focus:border-[#0C7B8E] bg-white text-gray-900 placeholder:text-gray-400 text-sm resize-none';
const labelClass = 'text-xs font-semibold text-gray-600 uppercase tracking-wide';
const switchClass = 'data-[state=checked]:bg-[#0C7B8E] data-[state=unchecked]:bg-gray-300 border-transparent';
...
export function AFPropertyForm({ data, onChange }: AFPropertyFormProps) {
  const set = (field: keyof AFPropertyData, value: unknown) =>
    onChange({ ...data, [field]: value });

  return (
    <div className="space-y-6">
      <Section title="Identificação" color="#0C7B8E">
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2 space-y-1">
            <Label className={labelClass}>Nome do imóvel / condomínio *</Label>
            <Input className={inputClass} placeholder="Ex: Residencial Mar Aberto"
      ...
      <Section title="Especificações" color="#0C7B8E">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="space-y-1">
            ...
            <Input className={inputClass} placeholder="Ex: 8"
      ...
      <Section title="Itens de Lazer" color="#0C7B8E">
        <div className="space-y-1">
          <Label className={labelClass}>Itens (um por linha ou separados por vírgula)</Label>
          <Textarea className={textareaClass} rows={2}
            placeholder={"Piscina com vista para o mar\nChurrasqueira\nSalão de festas"}
            value={data.leisureItems} onChange={(e) => set('leisureItems', e.target.value)} />
        </div>
      </Section>

      <Section title="Valores" color="#E8562A">
        <div className="flex items-center gap-1 p-1 bg-gray-100 rounded-xl w-full">
          <button type="button" onClick={() => set('isRental', false)}
            className="flex-1 py-2 rounded-lg text-sm font-semibold transition-all"
            style={!data.isRental ? { backgroundColor: '#E8562A', color: 'white', boxShadow: '0 1px 4px rgba(0,0,0,0.15)' } : { color: '#6B7280', backgroundColor: 'transparent' }}>
            Venda
          </button>
          <button type="button" onClick={() => set('isRental', true)}
            className="flex-1 py-2 rounded-lg text-sm font-semibold transition-all"
            style={data.isRental ? { backgroundColor: '#0C7B8E', color: 'white', boxShadow: '0 1px 4px rgba(0,0,0,0.15)' } : { color: '#6B7280', backgroundColor: 'transparent' }}>
            Locação
          </button>
        </div>
        <div className="space-y-3">
      ...
      <Section title="Corretor / Contato" color="#0C7B8E">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <Input className={inputClass} placeholder="(85) 99999-9999"
          ...
          <div className="col-span-2 space-y-1">
            ...
            <Input className={inputClass} placeholder="Ex: CRECI 12345 PF"
              value={data.creci} onChange={(e) => set('creci', e.target.value)} />
          </div>
        </div>
      </Section>

      <Section title="Mensagem Informativa Extra" color="#E8562A">
        <Textarea className={textareaClass} rows={2}
