...
const txLabel: Record<string, { label: string; color: string }> = {
  ...
  aluguel: { label: 'Aluguel', color: '#1B5EA6' },
...
export default function AMOlxCatalog() {
  ...
  const toggleActive = async (id: string, current: boolean) => {
    const { error } = await supabase.from('am_olx_listings').update({ is_active: !current }).eq('id', id);
  ...
  const removeListing = async (id: string) => {
    ...
    const { error } = await supabase.from('am_olx_listings').delete().eq('id', id);
  ...
  return (
    <AMLayout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-xl font-bold text-gray-900">Catálogo OLX</h1>
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={copyFeedUrl} className="text-white" style={{ backgroundColor: '#F47920' }}>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-sm text-gray-500">
              Nenhum imóvel no catálogo ainda. Use o botão "Postar no Instagram AM" e marque "Publicar também na OLX".
            </div>
          ) : (
              ...
              {listings.map((l) => {
                ...
                return (
                  <div key={l.id} className="p-4 flex items-center gap-4 hover:bg-gray-50 transition">
                    <img
                      src={l.photos?.[0] || '/placeholder.svg'}
                      alt={l.title}
