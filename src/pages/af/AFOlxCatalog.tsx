...
const FEED_URL = `https://${import.meta.env.VITE_SUPABASE_PROJECT_ID}.supabase.co/functions/v1/af-olx-feed`;

const PRIMARY = '#0C7B8E';
const ACCENT = '#E8562A';

const txLabel: Record<string, { label: string; color: string }> = {
  venda: { label: 'Venda', color: ACCENT },
  aluguel: { label: 'Aluguel', color: PRIMARY },
...
export default function AFOlxCatalog() {
  ...
  const toggleActive = async (id: string, current: boolean) => {
    const { error } = await supabase.from('af_olx_listings').update({ is_active: !current }).eq('id', id);
  ...
  const removeListing = async (id: string) => {
    ...
    const { error } = await supabase.from('af_olx_listings').delete().eq('id', id);
  ...
  return (
    <AFLayout>
      <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-xl font-bold text-gray-900">Catálogo OLX — Apartamentos Fortaleza</h1>
        ...
        <div className="mb-6 rounded-2xl p-5 bg-white border border-gray-200 shadow-sm">
          <div className="flex items-start gap-3 mb-3">
            <div className="p-2 rounded-lg" style={{ backgroundColor: '#FFF0EB' }}>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={copyFeedUrl} className="text-white" style={{ backgroundColor: ACCENT }}>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-10 text-center text-sm text-gray-500">
              Nenhum imóvel no catálogo ainda. Use o botão "Publicar no Canal Pro" na página do AF.
            </div>
          ) : (
