  ...
  try {
    ...
    while (url) {
      ...
      for (const conv of convs) {
        try {
          // Identificar o participante que NÃO é a conta VDH
          ...
          // Inserir mensagens (mais antigas primeiro)
          ...
          stats.errors++;
          log.push(`Erro conv ${conv.id}: ${e instanceof Error ? e.message : String(e)}`);
        }
      }
