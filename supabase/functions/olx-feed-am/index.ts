...
// Reads from public.am_olx_listings where is_active = true
...
function transactionTag(t: string): string {
  // VRSYNC: SALE / RENTAL — "lancamento" também publicamos como SALE com flag
...
${photosXml}
    ...
    <Location displayAddress="Street">
      ...
      <State abbreviation="${esc(p.state ?? 'AM')}">${esc(p.state === 'AM' ? 'Amazonas' : p.state)}</State>
...
<ListingDataFeed xmlns="http://www.vivareal.com/schemas/1.0/VRSync" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.vivareal.com/schemas/1.0/VRSync http://xml.vivareal.com/vrsync.xsd">
  <Header>
    <Provider>Apartamentos Manaus</Provider>
    <Email>contato@apartamentosmanaus.com</Email>
    <ContactName>Iury Sampaio</ContactName>
